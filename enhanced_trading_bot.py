...
from working_nse_scraper import working_nse_scraper
...
class RiskManager:
    ...
    def __init__(self, capital: float):
        ...
        self.max_risk_per_trade = 0.03  # 3% risk per trade for maximum profit
        self.max_portfolio_risk = 0.1  # 10% total portfolio risk
        self.max_positions = 6  # Maximum 6 positions for multiple trades
    ...
    def calculate_position_size(self, price: float, stop_loss: float, current_positions: int) -> int:
        ...
        # Lower minimum trade size for more opportunities
        if quantity * price < 3000:  # Minimum Rs.3000 trade (was Rs.8000)
            return 0
    ...
    def calculate_stop_loss(self, price: float, action: str, volatility: float = 0.02) -> float:
        ...
        stop_distance = price * max(volatility, 0.015)  # 1.5% stop for tighter risk control
    ...
    def calculate_target(self, price: float, stop_loss: float, action: str, risk_reward: float = 3.5) -> float:
        ...
        reward = risk * risk_reward  # 4:1 risk-reward for maximum profit
    ...
    def pre_market_analysis(self) -> Dict:
        ...
        for symbol in self.watchlist:
            try:
                ...
                if hasattr(self.data_provider, 'get_live_price'):
...
class EnhancedTradingBot:
    ...
    def __init__(self, capital: float = 50000):
        ...
        # Initialize components with working NSE scraper
        self.data_provider = working_nse_scraper
        ...
        for symbol in self.watchlist:
            ...
            try:
                ...
                if self.profit_maximizer:
                    market_conditions = {
                        'high_volatility': hist_data['close'].pct_change().std() > 0.03 if len(hist_data) > 10 else False
                    }
                    should_trade = self.profit_maximizer.should_trade_intelligent(
                        {'score': final_signal, 'confidence': signal_strength, 'quality': enhanced_signals.get('quality', 'fair') if 'enhanced_signals' in locals() else 'fair'},
                        market_conditions
                    )
                else:
                    # Fallback to aggressive thresholds
                    should_trade = abs(final_signal) > 0.05 and signal_strength > 0.2
                ...
                if should_trade:
                    ...
                    quantity = self.profit_maximizer.calculate_aggressive_position_size(
                        current_price, signal_strength, volatility, len(self.positions)
                    )
    ...
    def check_exit_conditions(self, position: Position) -> Tuple[bool, str]:
        ...
        # Time stop at 2:30 PM (enhanced)
        time_stop = datetime.strptime('14:30', '%H:%M').time()
        if current_time >= time_stop:
            return True, "Time Stop 2:30 PM"
        ...
        try:
            hist_data = self.data_provider.get_historical_data(position.symbol, 30)
            if not hist_data.empty:
                technical_signals = self.technical_analyzer.generate_signals(hist_data)
                if technical_signals['strength'] > 0.6:
                    position_direction = 1 if position.quantity > 0 else -1
                    signal_direction = 1 if technical_signals['signal'] > 0 else -1
                    
                    if position_direction != signal_direction and abs(technical_signals['signal']) > 0.5:
    ...
    def close_position(self, symbol: str, reason: str):
        ...
        if hasattr(self, 'performance_analytics') and self.performance_analytics:
    ...
    def start_trading(self):
        ...
        while self.is_running:
            try:
                ...
                time.sleep(60)  # Wait 1 minute between cycles for maximum execution speed
